        ...
        try:
            ...
            for item in selected_items:
                samples.append({
                    'id': len(samples),
                    'question': item['question'],
                    'answer': ast.literal_eval(item['possible_answers'])[0] if item['possible_answers'] else '',
                    'possible_answers': ast.literal_eval(item['possible_answers']),
                    'dataset': 'popqa',
            ...
            try:
                if current_size == len(batch_prompts):
                    ...
                    for i in range(0, len(batch_prompts), current_size):
                        ...
                        sub_responses = self._generate_single_batch(sub_batch, max_new_tokens)
                ...
                for prompt in batch_prompts:
                    try:
                        response = self._generate_single_batch([prompt], max_new_tokens)
...
class StandardEvaluationPipeline:
    ...
    def evaluate_single_model(self, model_path: str) -> Dict[str, Dict]:
        ...
        for dataset_name in self.args.datasets:
            if dataset_name in dataset_methods:
                try:
                    print(
                        f"\n开始评估 {dataset_name.upper()} (样本数: {self.args.num_samples}, tokens: {DATASET_CONFIG[dataset_name]['max_new_tokens']})")
                    eval_method = dataset_methods[dataset_name]
    ...
    def save_results(self):
        ...
        results_with_metadata = {
            'metadata': {
                ...
                'datasets_evaluated': self.args.datasets,
                'dataset_config': {dataset: DATASET_CONFIG[dataset] for dataset in self.args.datasets}
            },
    ...
    def save_detailed_results(self):
        ...
        for model_name, model_detailed_results in self.detailed_results.items():
            detailed_file = os.path.join(self.args.output_dir, f"downstream_results_{model_name}_{timestamp}.json")
    ...
    def print_summary(self):
        ...
        for model_name, scores in self.results.items():
            ...
            print("-" * 40)
            for dataset in self.args.datasets:
                result = scores.get(dataset, {})
                if isinstance(result, dict) and 'accuracy' in result:
                    accuracy = result['accuracy']
                    total = result.get('total', 'N/A')
                    correct = result.get('correct', 'N/A')
                    tokens = DATASET_CONFIG[dataset]['max_new_tokens']
                    print(f"  {dataset.upper():12} : {accuracy:.3f} ({correct}/{total}) [{tokens:3d} tokens]")
...
def main():
    ...
    print("=" * 80)
    print("新增功能:")
    print("1. ✓ 数据集可通过--datasets参数配置")
    print("2. ✓ 每个数据集使用专门的max_new_tokens")
    print("3. ✓ 智能的token分配策略")
    print("4. ✓ 保持所有原有修复功能")
    print("=" * 80)
    print(f"当前数据集配置:")
    for dataset in args.datasets:
